...
| **COCO_Loader.py** | `CocoDataset` class for loading COCO format datasets with PyTorch Dataset interface. | COCO annotation files (`_annotations.coco.json`), image directories, torchvision `CocoDetection`. | `training_objdet.py` via `load_datasets()`. | Dataset objects with images and targets in COCO format (XYWH boxes). |
| **COCO_Wrapper.py** | `COCOWrapper` for applying torchvision v2 transforms to COCO datasets while maintaining annotation consistency. | Base transforms, input size parameters, debug mode settings. | `setup_transforms()` in `training_objdet.py`. | Transform functions for training and evaluation pipelines. |
| **PASCAL_Loader.py** | `PascalDataset` class for loading Pascal VOC XML format datasets. | Pascal VOC XML annotation files, image directories, PIL for image loading. | `training_objdet.py` via `load_datasets()`. | Dataset objects with images and targets converted from XYXY to XYWH format. |
| **YOLO_Loader.py** | `YoloDataset` class for loading YOLO format datasets with normalized XYWH annotations. | YOLO `.txt` annotation files, image directories, normalized coordinates. | `training_objdet.py` via `load_datasets()`. | Dataset objects with images and targets in COCO-compatible format. |
| **optimizer.py** | Factory function `get_optimizer()` that creates PyTorch optimizers based on configuration. | Model parameters, `OptimizerConfig` with type and hyperparameters. | `setup_optimizer()` in `training_objdet.py`, `training_class.py`. | Configured `torch.optim.Optimizer` instances (Adam, AdamW, SGD, etc.). |
| **scheduler.py** | Factory function `get_scheduler()` that creates learning rate schedulers based on configuration. | Optimizer instance, `SchedulerConfig` with scheduler type and parameters. | `setup_scheduler()` in `training_objdet.py`, `training_class.py`. | Configured `torch.optim.lr_scheduler` instances (StepLR, ReduceLROnPlateau, etc.). |
| **training.py** | Streamlit GUI entry point for training configuration; launches `User_Interface.py`. | Streamlit installation, access to `User_Interface.py`. | User via command line (`python training.py`). | Browser-based configuration interface, calls to training pipelines. |
| **compare.py** | Streamlit GUI for comparing multiple trained models via TensorBoard; launches `evaluation.py`. | Trained model directories in `trained_models`, TensorBoard installation. | User via command line (`python compare.py`). | TensorBoard comparison interface with multi-model logdir specifications. |
| **evaluation.py** | Model comparison interface with TensorBoard integration; functions like `build_logdir_spec()`, `find_terminal_command()`. | Trained models in `trained_models`, terminal emulators (Linux), TensorBoard. | `compare.py` via Streamlit interface. | TensorBoard commands for multi-model comparison, terminal execution (Linux). |
