...
class SwinTinyObjectDetection(nn.Module):
    ...
    def __init__(self, num_classes=20, pretrained=True, grid_size=7):
        ...
        self.backbone = timm.create_model(
            "swin_tiny_patch4_window7_224", pretrained=pretrained, features_only=True, out_indices=[3]  # Return intermediate features  # Use last stage features (highest level)
        )
        ...
        self.detection_head = ObjectDetectionHead(in_channels=backbone_channels, num_classes=num_classes, grid_size=grid_size)
            ...
            for i in range(grid_size):
                for j in range(grid_size):
                    ...
                    if final_conf > confidence_threshold:
                        batch_detections.append({"bbox": [x_min.item(), y_min.item(), box_width.item(), box_height.item()], "score": final_conf.item(), "category_id": class_pred.item()})
...
def build_model(num_classes=20, config=None):
    ...
    model = SwinTinyObjectDetection(num_classes=num_classes, pretrained=pretrained, grid_size=7)
