...
def list_files(path: Path, suffix=".py") -> list[str]:
    try:
        return [p.stem for p in sorted(path.iterdir())
...
with col1:
    optimizer_type = st.selectbox("Optimizer Type",
...
if mode == "Transferlearning: selbstrainiertes Modell":
    ...
    with col1:
        ...
        if freezing_enabled:
            freezing_strategy = st.selectbox("Freezing Strategy",
                                           ["freeze_all_except_head", "freeze_early_layers",
                                            "freeze_backbone", "unfreeze_all", "custom_freeze"])
            ...
            if freezing_strategy == "custom_freeze":
                ...
                freeze_layers_input = st.text_input("Freeze Layers (comma separated)",
                                                   value="backbone.layer1, backbone.layer2")
                unfreeze_layers_input = st.text_input("Unfreeze Layers (comma separated)",
                                                     value="detection_head, backbone.layer4")
    ...
    with col2:
        ...
        backbone_lr_multiplier = st.number_input("Backbone LR Multiplier",
                                                min_value=0.001, max_value=10.0, value=0.1, format="%.3f")
        head_lr_multiplier = st.number_input("Head LR Multiplier",
                                           min_value=0.001, max_value=10.0, value=1.0, format="%.3f")
...
config = {
    ...
    "optimizer": {
        ...
        "adam": {
            "betas": [float(beta1) if optimizer_type in ["Adam", "AdamW"] else 0.9,
                     float(beta2) if optimizer_type in ["Adam", "AdamW"] else 0.999]
        },
        "sgd": {
            "momentum": float(momentum) if optimizer_type == "SGD" else 0.9
        }
    },
    "augmentation": {
        "file": augmentation if augmentation else ""
    },
    "model": {
        "type": task,
        ...
        "transfer_learning": {
            ...
            "freezing": {
                ...
                "custom": {
                    "freeze_layers": freeze_layers if mode == "Transferlearning: selbstrainiertes Modell" and freezing_enabled and freezing_strategy == "custom_freeze" else ["backbone.layer1", "backbone.layer2"],
                    "unfreeze_layers": unfreeze_layers if mode == "Transferlearning: selbstrainiertes Modell" and freezing_enabled and freezing_strategy == "custom_freeze" else ["detection_head", "backbone.layer4"]
                }
            },
            "lr": {
